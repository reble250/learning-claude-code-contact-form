...
## skill
今回のブラウザテスト実行のスキルを作成したい。場所はプロジェクトフォルダに作成して。

サポートファイルをおいた方が良い？

テストデータ作成のスキルを作成したい。シーダーで生成して。件数は引数で渡したい。

## agent
.claude/agents/cre-reviewer.mdというカスタムサブエージェントの定義ファイルを作成してください。
カスタムサブエージェントの定義に従うよう、ヘッダーと本文で構成すること

## ヘッダー
下記の項目を指定する

- 名前；core-reviewer
- 説明：コードの品質をレビューするサブエージェント
- 使うツール：読み取り専用のツールだけ許可

## 本文
下記の動作をするような本文とする

- レビュー項目：セキュリティ・パフォーマンス・可読性・ベストプラクティス
- 出力：問題点を重要度順にリストアップし、改善提案を添える
